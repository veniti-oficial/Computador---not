...
class ArduinoDataRead {
    ...
    }/*aqui ele vai gerar valores "falsos" caso o arduino não funcione */
    ...
    SetConnection() {
        SerialPort.list().then(listSerialDevices => {
            ...
            if (listArduinoSerial.length != 1) { /*Aqui e que se no vetor for diferente de 1 ele vai chamar a função fake_data pra gerar dados */
                this.fake_data();
                throw new Error("Arduino not found - Generating data")
            }else{
                console.log("Arduino found in the com %s", listArduinoSerial[0].comName);/*aqui caso ele encontre o arduino vai mostrar a porta que esse arduino está */
