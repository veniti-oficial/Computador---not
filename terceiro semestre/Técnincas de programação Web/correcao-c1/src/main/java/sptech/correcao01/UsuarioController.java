...
public class UsuarioController {
    ...
    public ResponseEntity<List<Usuario>> getUsuarios() {
        return usuarios.isEmpty() ? ResponseEntity.status(200).body(usuarios) : ResponseEntity.status(204).build();
    }
        ...
        for (Usuario usuarioAtual : usuarios) {
            if (usuarioAtual.getUsuario().equals(usuario)) {
                if (usuarioAtual.isAutenticado()) {
                    ...
                    return ResponseEntity.status(200).body(String.format("Logoff do usuário %s concluído", usuarioAtual.getNome()));
                } else {
                    return ResponseEntity.status(401).body(String.format("Usuário %s NÃO está autenticado", usuarioAtual.getNome()));
                }
            }
        }
    ...
    public ResponseEntity<String> logoffGeral() {
        ...
        return ResponseEntity.status(200).build();//"Todos os usuarios sofreram logoff!";
    }
